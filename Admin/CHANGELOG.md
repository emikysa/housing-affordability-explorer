# Housing Affordability Framework - Changelog

> **For Claude Code:** Append a dated summary of significant changes at the end of each session.

---

## 2026-02-06 (Session 17) - Risk Model (9th Dimension), Lever Expansion, CE Population

### Risk Framework — Teaching Page + Model Dimension
Created comprehensive risk support as the 9th model dimension:

**Risk Teaching Page (`/risk`):**
- New educational page explaining the R1–R4 risk framework
- Core Principle: "Risk is not a cost itself. Risk is priced through time, capital, buffers, and transaction friction."
- Four sections (R1 Schedule, R2 Capital, R3 Scope, R4 Market) with definitions, affected cost elements, compounding explanation, municipal leverage insight
- Displays active risk model parameters and all presets
- Red color scheme

**Database: `risk_models` table + 4 presets:**
| Name | R1 Schedule | R2 Rate | R3 Design | R3 Constr | R4 Mktg | R4 Sales |
|------|------------|---------|-----------|-----------|---------|----------|
| Low Risk | 0% | 0 bps | 3% | 5% | 1.0x | 0 mo |
| Medium Risk | 10% | 50 bps | 5% | 8% | 1.2x | 2 mo |
| High Risk | 25% | 150 bps | 8% | 12% | 1.5x | 4 mo |
| Very High Risk | 40% | 300 bps | 12% | 18% | 2.0x | 8 mo |

**Frontend integration:**
- `RiskContext.tsx` + `RiskSelector.tsx` (red color scheme)
- Models page: Risk tab with 4 model cards, detail panel with R1–R4 sections
- Dashboard: Risk row in selector table, risk-adjusted mortgage rate (R2 premium), risk cost line item in total
- ModelSummaryBar: Risk chip; ModelConfigModal: Risk section
- Risk calculations: R1→schedule carry cost, R2→rate premium on mortgage, R3→contingency amortization, R4→marketing addon + sales carry

### Lever Expansion (5 → 20)
Added 15 new levers via migration:
- LEV-FEE_WAIVER, LEV-EXPEDITED_PERMIT, LEV-DENSITY_BONUS, LEV-INCLUSIONARY, LEV-LAND_TRUST
- LEV-TIF, LEV-WEATHERIZATION, LEV-PREAPPROVED_PLANS, LEV-MODULAR, LEV-WORKFORCE
- LEV-ADU_REFORM, LEV-INFRA_SHARING, LEV-PUBLIC_LAND, LEV-GREEN_INCENTIVE, LEV-TOD
- 58 new barrier_lever_map entries (total now 129)

### Files Created
- `supabase/migrations/20260206200000_expand_levers.sql`
- `supabase/migrations/20260206210000_risk_models.sql`
- `frontend/src/contexts/RiskContext.tsx`
- `frontend/src/components/RiskSelector.tsx`
- `frontend/src/pages/Risk.tsx`

### Files Modified
- `frontend/src/types/database.ts` — added RiskModel interface
- `frontend/src/hooks/useData.ts` — added useRiskModels() hook
- `frontend/src/pages/Models.tsx` — added Risk tab, detail panel, active models chip
- `frontend/src/pages/Dashboard.tsx` — added Risk row, risk-adjusted calculations
- `frontend/src/components/ModelSummaryBar.tsx` — added Risk chip
- `frontend/src/components/ModelConfigModal.tsx` — added Risk section
- `frontend/src/components/Layout.tsx` — added Risk nav item
- `frontend/src/App.tsx` — added /risk route
- `frontend/src/main.tsx` — added RiskProvider
- `Admin/CONTEXT.md` — updated model dimensions, data counts, file structure
...
|--------|--------|-------|
| Levers | 5 | 20 |
| Barrier-Lever mappings | 71 | 129 |
| Risk Model Presets | 0 | 4 |
| Model Dimensions | 8 | 9 |

### Deployed: Pending (build succeeds, needs git push)
...
### New Overview Page
Created `/overview` as educational landing page:
- Framework explanation ("What is this tool?")
- Visual chain diagram: Costs → Opportunities → Barriers → Levers → Actors
- Framework counts using StatCard components
- Descriptions for all navigation tabs
- Beta warning notice
- "Go to Dashboard" CTA button

### Dashboard Streamlined
Removed educational content, focused on calculator:
- Kept: Monthly Housing Cost Calculator (model selectors + results)
- Kept: Cost Model Analysis (summary + stacked bar charts)
- Kept: Quick Links (added link to Overview)
- Removed: Framework Overview stat cards
- Removed: Barrier analysis pie charts

### Header UX Improvements
- Moved version stamp from individual pages to common header (after tagline)
- Added Beta badge next to main title
- Removed VersionStamp imports from all 9 page files

### Dashboard Model Table Cleanup
- Removed redundant labels from model selector dropdowns
- Narrowed Selection column (w-52), widened Details column
- Enhanced Details text with more context

### Files Created
- `frontend/src/pages/Overview.tsx`

### Files Modified
- `frontend/src/App.tsx` - Added Overview route
- `frontend/src/components/Layout.tsx` - Added Overview nav, version stamp to header, Beta badge
- `frontend/src/pages/Dashboard.tsx` - Removed framework overview and barrier charts
- All 9 page files - Removed VersionStamp imports
...
- Built full Housing Affordability Framework application
