# Housing Affordability Framework - Project Context
...
│   │   │   ├── ModelConfigModal.tsx   # Full modal for all 9 model selectors
...
│   │   │   ├── RiskSelector.tsx       # Dropdown for risk model
│   │   │   ├── FilterToggle.tsx      # "Populated only / Show all" toggle
│   │   │   └── DetailPanel.tsx       # Slide-out detail panels
│   │   ├── contexts/                # Global model state management
│   │   │   ├── ModelContext.tsx      # Cost model context
│   │   │   ├── OccupancyContext.tsx  # Occupancy model context
│   │   │   ├── LifestyleContext.tsx  # Lifestyle model context
│   │   │   ├── UtilityContext.tsx    # Water/Sewer/Electric/Gas contexts
│   │   │   ├── FinanceContext.tsx    # Finance model context
│   │   │   └── RiskContext.tsx       # Risk model context (R1-R4)
│   │   ├── pages/
...
│   │   │   ├── Models.tsx             # Model management cards (9 tabs)
│   │   │   ├── Explorer.tsx          # 5-column interactive explorer
│   │   │   └── Risk.tsx              # Risk teaching page (R1-R4)
...
- **10+ views:** v_cost_elements, v_cros, v_barriers, v_levers, v_barrier_levers, v_barrier_cros, v_risk_models, etc.
...
- 20 levers (5 original + 15 expanded), 129 barrier-lever mappings
...
### Data Counts (Updated 2026-02-06)
| Entity | Count |
|--------|-------|
...
| Barrier-Lever mappings | 129 |
| Risk Model Presets | 4 (Low, Medium, High, Very High) |
...
### Navigation Tabs
Overview, Dashboard, Explorer, Models, Costs, Opportunities, Barriers, Levers, Actors, Relationships, Risk
...
### Dashboard (Calculator Focus)
Streamlined to focus on the Monthly Housing Cost Calculator:
1. **Monthly Housing Cost Calculator** - 9 model selectors in table format + results (including Risk)
2. **Cost Model Analysis** - Summary stats and stacked bar charts
3. **Quick Links** - Navigation to other pages (including Overview)

### Risk Teaching Page
Educational page explaining the R1–R4 risk framework:
- Core Principle: "Risk is not a cost itself. Risk is priced through time, capital, buffers, and transaction friction."
- Four sections: R1 Schedule, R2 Capital, R3 Scope, R4 Market — each with definition, affected cost elements, compounding explanation, and municipal leverage insight
- Displays active risk model preset parameters
- Red color scheme consistent with risk model UI elements
...
### Version Stamp
- **Version stamp displayed in common header** (after tagline "Understanding costs, opportunities, and barriers")
...
└── Risk Model: "Medium Risk"
```

### Model Dimensions (9 Dimensions)
...
| **Risk** | Risk parameters R1–R4 | Schedule carry, rate premium, contingency, marketing |
...
Build Cost × Finance Terms + Risk Rate Premium = Monthly Mortgage (F01-F03)
Risk Model → Schedule carry + Contingency + Marketing addon = Risk Costs
...
**Phase 6: Risk Model (9th Dimension)** (COMPLETE - 2026-02-06)
- ✓ `risk_models` table with R1–R4 parameters + 4 presets (Low/Medium/High/Very High)
- ✓ `v_risk_models` view
- ✓ `RiskContext` + `RiskSelector` component (red color scheme)
- ✓ `/risk` teaching page explaining R1–R4 framework conceptually
- ✓ Models page: Risk tab with model cards showing R1–R4 parameters
- ✓ Dashboard: Risk row in model selectors, risk-adjusted mortgage rate (R2), risk cost line item
- ✓ ModelSummaryBar + ModelConfigModal: Risk chip and selector
- ✓ Risk calculation: R1 schedule carry, R2 rate premium, R3 contingency, R4 marketing/sales carry

**Phase 7: Lever Expansion** (COMPLETE - 2026-02-06)
- ✓ 15 new levers added (total now 20), covering: fee waivers, expedited permits, density bonuses, inclusionary zoning, land trusts, TIF, weatherization, pre-approved plans, modular construction, workforce development, ADU reform, infrastructure sharing, public land, green incentives, TOD
...
risk_models (
  ...
  name text,
  description text,
  schedule_variance_pct numeric DEFAULT 0,     -- R1: Schedule uncertainty (%)
  rate_premium_bps numeric DEFAULT 0,          -- R2: Cost-of-capital premium (basis points)
  design_contingency_pct numeric DEFAULT 0,    -- R3: Design contingency (%)
  construction_contingency_pct numeric DEFAULT 0, -- R3: Construction contingency (%)
  marketing_multiplier numeric DEFAULT 1.0,    -- R4: Marketing cost multiplier
  sales_period_months numeric DEFAULT 0,       -- R4: Additional sales carry months
  sort_order integer
)
```
