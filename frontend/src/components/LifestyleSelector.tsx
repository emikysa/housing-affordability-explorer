...
export default function LifestyleSelector({
  ...
  const { lifestyleModels, selectedLifestyleModelId, setSelectedLifestyleModelId, loading } = useLifestyle()
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      ...
      <select
        id="lifestyle-select"
        value={selectedLifestyleModelId}
        onChange={(e) => setSelectedLifestyleModelId(e.target.value)}
        className={`
